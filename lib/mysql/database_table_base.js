...
module.exports = class {
  ...
  */
   ...
   */
  *getConnection() {
    return yield this._client.getConnection();
  }
  ...
  *add( item ) {
    ...
    try {
      if (!conn)  { conn = yield febs.utils.denodeify(this.client.getConnection, this.client)(); needRelease = true; }
      if (!conn)  return false;
    } catch (e) { return false; }

    try {
      var ret = yield febs.utils.denodeify(conn.query, conn)({sql:sql, timeout:this.queryTimeout});
      if (ret && ret.insertId) {
  ...
  *queryWhere( where ) {
    ...
    {
      try {
        conn = yield febs.utils.denodeify(this.client.getConnection, this.client)(); needRelease = true;
        if (!conn)  return null;
      } catch (e) { return null; }
    }
