...
* @return: (Promise).
*/
async function transaction_foo(db, fooCB, commitCB, rollbackCB, resolve, reject, conn, needCommit) {  
  try {
...
function transaction(db, isolationLevel1, fooCB, commitCB, rollbackCB) {
  ...
  return new PromiseLib((resolve, reject)=>{
    ...
    if (db._transactionConn) {
      ...
      transaction_foo(db, fooCB, commitCB, rollbackCB, resolve, reject, db._transactionConn, false);
    }
    else {
      ...
      db.pool.getConnection((err, conn)=>{
        ...
        conn.query({sql:sqlLevel, timeout:db.queryTimeout}, (err, ret)=>{
          ...
          conn.query({sql:'START TRANSACTION', timeout:db.queryTimeout}, (err, ret)=>{
            ...
            transaction_foo(db, fooCB, commitCB, rollbackCB, resolve, reject, conn, true);
          });
