...
* @return: (Promise).
*/
async function transaction_foo(db, fooCB, preCB, commitCB, rollbackCB, resolve, reject, conn, needCommit) {  
...
function transaction(db, isolationLevel1, fooCB, preCB, commitCB, rollbackCB) {
  ...
  return new PromiseLib((resolve, reject)=>{
    ...
    if (db._transactionConn) {
      ...
      transaction_foo(db, fooCB, preCB, commitCB, rollbackCB, resolve, reject, db._transactionConn, false);
    }
    else {
      ...
      db.pool.getConnection((err, conn)=>{
        ...
        conn.query({sql:sqlLevel, timeout:db.queryTimeout}, (err, ret)=>{
          ...
          conn.query({sql:'START TRANSACTION', timeout:db.queryTimeout}, (err, ret)=>{
            ...
            transaction_foo(db, fooCB, preCB, commitCB, rollbackCB, resolve, reject, conn, true);
          });
