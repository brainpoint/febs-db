...
function ret_data_cvt(rows, table) {
  for (let i = 0; i < rows.length; i++) {
    for (let k in rows[i]) {
      let kk = table.model[k];
      if (kk) {
        let t = dataType.getType(kk.type);
        // fix bigint col.
        if (typeof rows[i][k] === 'string' && t === dataType.BigInt) {
          if (rows[i][k].length >= 11)
            rows[i][k] = new BigNumber(rows[i][k]);
          else
            rows[i][k] = parseInt(rows[i][k]);
        }
        // fix boolean col.
        else if (t === dataType.Bit) {
          rows[i][k] = (1==rows[i][k].readUInt8(0));
        }
      }
    }
  }
  ...
  return new PromiseLib((resolve, reject)=>{
    ...
    try {
      ...
      for (let i = 0; i < params.params.length; i++) {
        ...
        if (pi.in) {
          paramSql += type_cast(pi.value, pi.type);
        } else {
          paramSql += '@a' + i;
        }
      } // if.
      ...
      db.pool.getConnection((err, conn)=>{
        ...
        conn.query({sql:sqlLevel, timeout:db.queryTimeout}, (err, ret)=>{
          ...
          conn.query({sql:'START TRANSACTION', timeout:db.queryTimeout}, (err, ret)=>{
            console.debug('[Begin Transaction] ' + (err?'err':'ok') + ' ' + isolationLevel1);
            try {
  ...
  if (table.idKeyIsCombined) {
    for (let i = 0; i < table.idKeyName.length; i++) {
      ...
      if (id2 === undefined || id2 === null) {
        ...
        if (i > 0)
          sqlprimary += ' AND ';
        sqlprimary += "`" + table.idKeyName[i] + "`=" + type_cast(id2, table.model[table.idKeyName[i]].type);
      }
    }
  } else {
    let id2 = id;
    if (id2 !== undefined && id2 !== null) {
      sqlprimary += "`" + table.idKeyName + "`=" + type_cast(id2, table.model[table.idKeyName].type);
    } else {
      sqlprimary = null;
    }
  }
  ...
  if (table.idKeyIsCombined) {
    for (let i = 0; i < table.idKeyName.length; i++) {
      ...
      if (id === undefined || id === null) {
        ...
        if (i > 0)
          sqlprimary += ' AND ';
        sqlprimary += "`" + table.idKeyName[i] + "`=" + type_cast(id, table.model[name].type);
      }
    }
  } else {
    let id = item[table.idKeyName];
    if (id !== undefined && id !== null) {
      sqlprimary += "`" + table.idKeyName + "`=" + type_cast(id, table.model[table.idKeyName].type);
    }
  }
