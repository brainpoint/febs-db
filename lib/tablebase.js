...
* @param model: 本表模型.
*/
function init(tablename, idKeyName, model) {
...
function tablebase(tablename, idKeyName, model) {
  if (tablename) {
    init.bind(this)(tablename, idKeyName, model);
  }
}
...
* @param model: 本表模型.
*/
tablebase.prototype.init = function (tablename, idKeyName, model) {
  init.bind(this)(tablename, idKeyName, model);
}
...
*   ret - boolean.
*/
tablebase.prototype.remove = function (where) {
  if (typeof where === 'object') {
    ...
    for (let key of keys) {
      ...
      whereStr += this._condition.equal(key, where[key]);
    }
    where = whereStr;
  }
  return this.adapter.remove(this, where);
}
...
*   ret - boolean.
*/
tablebase.prototype.update = function (item, where = null) {
  ...
  return this.adapter.update(this, item, where);
}
...
*   ret - mod array.
*/
tablebase.prototype.select = function (where, opt = null) {
  if (where && typeof where === 'object') {
    let keys = Object.keys(where);
    let whereStr = '';
    for (let key of keys) {
  ...
  return this.adapter.select(this, where, opt);
}
...
*   ret - number.
*/
tablebase.prototype.count = function (where = null) {
  if (where && typeof where === 'object') {
    let keys = Object.keys(where);
    let whereStr = '';
    for (let key of keys) {
...
* @desc: 返回join对象.
*/
tablebase.prototype.join_inner = function(tableB) { return new join('INNER JOIN', this.adapter, this, tableB); }
tablebase.prototype.join_cross = function(tableB) { return new join('CROSS JOIN', this.adapter, this, tableB); }
tablebase.prototype.join_left = function(tableB) { return new join('LEFT OUTER JOIN', this.adapter, this, tableB); }
tablebase.prototype.join_right = function(tableB) { return new join('RIGHT OUTER JOIN', this.adapter, this, tableB); }
tablebase.prototype.join_full = function(tableB) { return new join('FULL OUTER JOIN', this.adapter, this, tableB); }
