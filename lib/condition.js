...
module.exports = class {
  ...
  */
  equal( key, value, alias = null ) {
    ...
    return ` ${alias?alias+'.':''}${key}=${this.adapter.type_cast(value, t.type)} `;
  }
  ...
  * @return: sql;
  */
  less_then( key, value, alias = null ) {
   var t = this.model[key];
   if (!t)
      throw new exception(key + ' is not existed', febs.exception.PARAM, __filename, __line);
  ...
  */
  like( key, value, alias = null ) {
    ...
    {
      return ` ${alias?alias+'.':''}${key} LIKE '${value}' `;
    }
    else
    {
  ...
  */
  between( key, value1, value2, alias = null ) {
    ...
    return ` ${alias?alias+'.':''}${key} BETWEEN ${this.adapter.type_cast(value1, t.type)} AND ${this.adapter.type_cast(value2, t.type)} `;
  }
  ...
  */
  in( key, valueArray, alias = null ) {
    ...
    let sql = ` ${alias?alias+'.':''}${key} IN (`;

    for (let i = 0; i < valueArray.length; i++) {
      if (i > 0)
        sql += ',';
      sql += this.adapter.type_cast(valueArray[i], t.type);
    }
    sql += ') ';
    return sql;
  }
  ...
  */
  not_in( key, valueArray, alias = null ) {
    ...
    let sql = ` ${alias?alias+'.':''}${key} NOT IN (`;
