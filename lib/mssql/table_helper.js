  ...
  if (table._idKeyIsCombined) {
    for (let i = 0; i < table._idKeyName.length; i++) {
      ...
      if (id2 === undefined || id2 === null) {
        ...
        if (i > 0)
          sqlprimary += ' AND ';
        sqlprimary += alias + table._idKeyNameMap[i] + "=" + type_cast(id2, table._model[name].type, alias);
      }
    }
  } else {
    let id2 = id;
    if (id2 !== undefined && id2 !== null) {
      sqlprimary += alias + table._idKeyNameMap + "=" + type_cast(id2, table._model[table._idKeyName].type);
    }
  }
