...
* @return: 
*/
async function do_transaction(db, fooCB, commitCB, rollbackCB, resolve, reject, rejectCb, conn, needCommit) {  
  try {
...
* @return: (Promise).
*/
function transaction(db, isolationLevel1, fooCB, commitCB, rollbackCB) {
  assert(db);
  ...
  return new PromiseLib((resolve, reject)=>{
    ...
    if (db._transactionConn) {
      ...
      do_transaction(db, fooCB, commitCB, rollbackCB, resolve, reject, null, db._transactionConn, false);
    }
    else {
      ...
      try {
          ...
          try {
            pool.connect(err=>{
              ...
              trans_conn.begin(sqlLevel, err=>{
                ...
                do_transaction(db1, fooCB, commitCB, rollbackCB, resolve, reject, err=>{
                }, trans_conn, true);
              });
        ...
        // query.
          ...
          trans_conn.begin(sqlLevel, err=>{
            ...
            do_transaction(db1, fooCB, commitCB, rollbackCB, resolve, reject, null, trans_conn, true);
          });
        } // if..else.
      } catch (err) {
