  ...
  for (let i = 0; i < rows.length; i++) {
    for (let k in rows[i]) {
      ...
      if (kk) {
        ...
        // fix datetime.
        else if (rows[i][kk.map] instanceof Date) {
          let localDate = Date.UTC(
                            rows[i][kk.map].getUTCFullYear(),
                            rows[i][kk.map].getUTCMonth(), 
                            rows[i][kk.map].getUTCDate(), 
                            rows[i][kk.map].getUTCHours(), 
                            rows[i][kk.map].getUTCMinutes(), 
                            rows[i][kk.map].getUTCSeconds());
...
* @return: 
*/
async function do_transaction(db, fooCB, resolve, reject, rejectCb, conn, needCommit) {  
  try {
...
function transaction(db, isolationLevel1, fooCB) {
  ...
  return new PromiseLib((resolve, reject)=>{
    ...
    if (db._transactionConn) {
      ...
      do_transaction(db, fooCB, resolve, reject, null, db._transactionConn, false);
    }
    else {
      ...
      try {
        ...
        // to connect pool.
          ...
          pool.connect(err=>{
            ...
            trans_conn.begin(sqlLevel, err=>{
              ...
              do_transaction(db1, fooCB, resolve, reject, err=>{
              }, trans_conn, true);
            });
        ...
        // query.
          ...
          trans_conn.begin(sqlLevel, err=>{
            ...
            do_transaction(db1, fooCB, resolve, reject, null, trans_conn, true);
          });
        } // if..else.
      } catch (err) {
